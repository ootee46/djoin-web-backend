...
export class DashboardUpcomingComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    this.listData$ = this._service.upcomingMeetings$;
  }
  ...
  loadData(): void {
    this.searchQuery.pageSize = (this.paginator ? this.paginator.pageSize : this.searchQuery.pageSize);
    this._service.getUpcommingMeeting(this.searchQuery).pipe(take(1)).subscribe(() => {
      if (this.paginator) {
        if (!this._isPageLoad) {
          this._isPageLoad = true;
          this.paginator.page.pipe(takeUntil(this._unsubscribeAll)).subscribe(() => {
