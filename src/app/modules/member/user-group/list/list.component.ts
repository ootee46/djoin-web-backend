...
import { UserGroupListModel, UserGroupModel } from 'app/models/user-group.model';
...
export class UserGroupListComponent implements OnInit, OnDestroy, AfterViewInit {
    @ViewChild('paginator', { static: false }) paginator: MatPaginator;
    @ViewChild('scrollTable', { static: false }) scroll: ElementRef;
    listData$: Observable<DataListModel<UserGroupListModel>>;
    isLoading: boolean = false;
    searchQuery: QueryListModel = new QueryListModel({});
    private _formPopup: MatDialogRef<UserGroupFormComponent>;
    ...
    create(): void {
        const inputData: InputFormData<UserGroupModel> = new InputFormData<UserGroupModel>();
        inputData.data = new UserGroupModel({});
        inputData.action = 'add';
        this.openForm(inputData);
    }
    edit(rowData: UserGroupListModel): void {
        const inputData: InputFormData<UserGroupModel> = new InputFormData<UserGroupModel>();
        this._service.getData(rowData.id).pipe(takeUntil(this._unsubscribeAll)).subscribe((response: UserGroupModel) => {
            inputData.data = response;
            inputData.action = 'edit';
            this.openForm(inputData);
        });
    }
    deleteData(rowData: UserGroupListModel): void {
        Swal.fire({
            title: '',
    ...
    openForm(input: InputFormData<UserGroupModel>): void {
        this._formPopup = this._matDialog.open(UserGroupFormComponent, {
