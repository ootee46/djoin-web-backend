...
import { MeetingRoomListModel, MeetingRoomModel } from 'app/models/meeting-room.model';


@Component({
    selector: 'meeting-room-list',
    templateUrl: './list.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations,
})
export class MeetingRoomListComponent implements OnInit, OnDestroy, AfterViewInit {
    @ViewChild('paginator', { static: false }) paginator: MatPaginator;
    @ViewChild('scrollTable', { static: false }) scroll: ElementRef;
    listData$: Observable<DataListModel<MeetingRoomListModel>>;
    isLoading: boolean = false;
    searchQuery: QueryListModel = new QueryListModel({});
    private _formPopup: MatDialogRef<MeetingRoomFormComponent>;
    ...
    create(): void {
        const inputData: InputFormData<MeetingRoomModel> = new InputFormData<MeetingRoomModel>();
        inputData.data = new MeetingRoomModel({});
        inputData.action = 'add';
        this.openForm(inputData);
    }
    edit(rowData: MeetingRoomListModel): void {
        const inputData: InputFormData<MeetingRoomModel> = new InputFormData<MeetingRoomModel>();
        this._service.getData(rowData.id).pipe(takeUntil(this._unsubscribeAll)).subscribe((response: MeetingRoomModel) => {
            inputData.data = response;
            inputData.action = 'edit';
            this.openForm(inputData);
        });
    }
    deleteData(rowData: MeetingRoomListModel): void {
        Swal.fire({
            title: '',
    ...
    openForm(input: InputFormData<MeetingRoomModel>): void {
        this._formPopup = this._matDialog.open(MeetingRoomFormComponent, {
