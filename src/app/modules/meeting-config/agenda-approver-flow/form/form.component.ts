...
@Component({
    selector: 'agenda-approver-flow-form',
    templateUrl: './form.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations,
})
export class AgendaApproverFlowFormComponent implements OnInit, OnDestroy {
    @ViewChild('myForm') myForm: NgForm;
    action: string;
    dialogTitle: string;
    data: AgendaApproverFlowModel;
    dataForm: UntypedFormGroup;
    kw: string;
    private _unsubscribeAll: Subject<any> = new Subject();
    constructor(
        private _matDialogRef: MatDialogRef<AgendaApproverFlowFormComponent>,
        @Inject(MAT_DIALOG_DATA) private input: InputFormData<AgendaApproverFlowModel>,
    ...
    saveData(): void {
        if (this.dataForm.valid) {
            const postData = new AgendaApproverFlowModel(this.dataForm.getRawValue());
            Swal.fire({
                title: '',
