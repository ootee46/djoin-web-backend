...
export class VenueListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  listData$: Observable<DataListModel<StandardModel>>;
  isLoading: boolean = false;
  searchQuery: QueryListModel = new QueryListModel({});
  private _formPopup: MatDialogRef<VenueFormComponent>;
  ...
  create(): void {
    const inputData: InputFormData<StandardModel> = new InputFormData<StandardModel>();
    inputData.data = new StandardModel({});
    inputData.action = 'add';
    this.openForm(inputData);
  }

  edit(rowData: StandardModel): void {
    const inputData: InputFormData<StandardModel> = new InputFormData<StandardModel>();
    this._service.getData(rowData.id).pipe(takeUntil(this._unsubscribeAll)).subscribe((response: StandardModel) => {
  ...
  deleteData(rowData: StandardModel): void {
    Swal.fire({
      title: '',
  ...
  openForm(input: InputFormData<StandardModel>): void {
    this._formPopup = this._matDialog.open(VenueFormComponent, {
      panelClass: 'site-dialog',
      width: '100%',
      data: input
    });
