...
export class ApproverStepListComponent implements OnInit, OnDestroy, AfterViewInit {
    @ViewChild('paginator', { static: false }) paginator: MatPaginator;
    @ViewChild('scrollTable', { static: false }) scroll: ElementRef;
    listData$: Observable<DataListModel<ApproverStepModel>>;
    isLoading: boolean = false;
    searchQuery: QueryListModel = new QueryListModel({});
    private _formPopup: MatDialogRef<ApproverStepFormComponent>;
    ...
    create(): void {
        const inputData: InputFormData<ApproverStepModel> = new InputFormData<ApproverStepModel>();
        inputData.data = new ApproverStepModel({});
        inputData.action = 'add';
        this.openForm(inputData);
    }
    edit(rowData: ApproverStepModel): void {
        const inputData: InputFormData<ApproverStepModel> = new InputFormData<ApproverStepModel>();
        this._service.getData(rowData.id).pipe(takeUntil(this._unsubscribeAll)).subscribe((response: ApproverStepModel) => {
            inputData.data = response;
            inputData.action = 'edit';
            this.openForm(inputData);
        });
    }
    deleteData(rowData: ApproverStepModel): void {
        Swal.fire({
            title: '',
    ...
    openForm(input: InputFormData<ApproverStepModel>): void {
        this._formPopup = this._matDialog.open(ApproverStepFormComponent, {
