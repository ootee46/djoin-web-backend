...
@Component({
    ...
    styles         : [
        /* language=SCSS */
        `
            .approver-config-grid {
                grid-template-columns: 80px auto 150px 150px 180px 180px;
            }
        `
    ],
})
export class ApproverConfigListComponent implements OnInit, OnDestroy, AfterViewInit {
    @ViewChild('paginator', { static: false }) paginator: MatPaginator;
    @ViewChild('scrollTable', { static: false }) scroll: ElementRef;
    listData$: Observable<DataListModel<ApproverConfigModel>>;
    isLoading: boolean = false;
    searchQuery: QueryListModel = new QueryListModel({});
    private _formPopup: MatDialogRef<ApproverConfigFormComponent>;
    ...
    create(): void {
        const inputData: InputFormData<ApproverConfigModel> = new InputFormData<ApproverConfigModel>();
        inputData.data = new ApproverConfigModel({});
        inputData.action = 'add';
        this.openForm(inputData);
    }
    edit(rowData: ApproverConfigModel): void {
        const inputData: InputFormData<ApproverConfigModel> = new InputFormData<ApproverConfigModel>();
        this._service.getData(rowData.id).pipe(takeUntil(this._unsubscribeAll)).subscribe((response: ApproverConfigModel) => {
            inputData.data = response;
            inputData.action = 'edit';
            this.openForm(inputData);
        });
    }
    duplicate(rowData: ApproverConfigModel): void {
        const inputData: InputFormData<ApproverConfigModel> = new InputFormData<ApproverConfigModel>();
        this._service.getData(rowData.id).pipe(takeUntil(this._unsubscribeAll)).subscribe((response: ApproverConfigModel) => {
            inputData.data = response;
            inputData.action = 'add';
    ...
    deleteData(rowData: ApproverConfigModel): void {
        Swal.fire({
            title: '',
    ...
    openForm(input: InputFormData<ApproverConfigModel>): void {
        this._formPopup = this._matDialog.open(ApproverConfigFormComponent, {
