...
@Component({
    selector: 'approver-position-form',
    templateUrl: './form.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations,
})
export class ApproverPositionFormComponent implements OnInit, OnDestroy {
    @ViewChild('myForm') myForm: NgForm;
    action: string;
    dialogTitle: string;
    data: ApproverPositionModel;
    dataForm: UntypedFormGroup;
    kw: string;
    private _unsubscribeAll: Subject<any> = new Subject();
    constructor(
        private _matDialogRef: MatDialogRef<ApproverPositionFormComponent>,
        @Inject(MAT_DIALOG_DATA) private input: InputFormData<ApproverPositionModel>,
    ...
    saveData(): void {
        if (this.dataForm.valid) {
            const postData = new ApproverPositionModel(this.dataForm.getRawValue());
            Swal.fire({
                title: '',
