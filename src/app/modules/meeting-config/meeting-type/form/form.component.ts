...
@Component({
    selector: 'meeting-type-form',
    templateUrl: './form.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations,
})
export class MeetingTypeFormComponent implements OnInit, OnDestroy {
    @ViewChild('myForm') myForm: NgForm;
    action: string;
    dialogTitle: string;
    data: MeetingTypeModel;
    dataForm: UntypedFormGroup;
    kw: string;
    private _unsubscribeAll: Subject<any> = new Subject();
    constructor(
        private _matDialogRef: MatDialogRef<MeetingTypeFormComponent>,
        @Inject(MAT_DIALOG_DATA) private input: InputFormData<MeetingTypeModel>,
    ...
    createDataForm(): UntypedFormGroup {
        return this._formBuilder.group({
            ...
            'title': [this.data.title, [Validators.required, Validators.maxLength(90), CustomValidator.whiteSpace]],
            'active': [this.data.active, [Validators.required]]
        });
    }
    saveData(): void {
        if (this.dataForm.valid) {
            const postData = new MeetingTypeModel(this.dataForm.getRawValue());
            Swal.fire({
                title: '',
