...
@Component({
    selector: 'meeting-sub-type-list',
    templateUrl: './list.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations
})
export class MeetingSubTypeListComponent implements OnInit, OnDestroy, AfterViewInit {
    @ViewChild('paginator', { static: false }) paginator: MatPaginator;
    @ViewChild('scrollTable', { static: false }) scroll: ElementRef;
    listData$: Observable<DataListModel<MeetingSubTypeModel>>;
    isLoading: boolean = false;
    searchQuery: QueryListModel = new QueryListModel({});
    catData: MeetingTypeModel = new MeetingTypeModel({});
    private _formPopup: MatDialogRef<MeetingSubTypeFormComponent>;
    ...
    ngOnInit(): void {
        ...
        this.catData = this._service.getCatData();
        this.searchQuery.catId = this.catData.id;
    }
    ...
    create(): void {
        const inputData: InputFormData<MeetingSubTypeModel> = new InputFormData<MeetingSubTypeModel>();
        inputData.data = new MeetingSubTypeModel({});
        inputData.data.meetingTypeId = this.catData.id;
        inputData.action = 'add';
        this.openForm(inputData);
    }
    edit(rowData: MeetingSubTypeModel): void {
        const inputData: InputFormData<MeetingSubTypeModel> = new InputFormData<MeetingSubTypeModel>();
        inputData.data = rowData;
        inputData.action = 'edit';
        this.openForm(inputData);
    }
    deleteData(rowData: MeetingSubTypeModel): void{
        Swal.fire({
            title: '',
    ...
    openForm(input: InputFormData<MeetingSubTypeModel>): void{
        this._formPopup = this._matDialog.open(MeetingSubTypeFormComponent, {
