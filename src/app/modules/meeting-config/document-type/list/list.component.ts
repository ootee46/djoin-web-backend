...
import { DocumentTypeListModel, DocumentTypeModel } from 'app/models/document-type.model';
...
export class DocumentTypeListComponent implements OnInit, OnDestroy, AfterViewInit {
    @ViewChild('paginator', { static: false }) paginator: MatPaginator;
    @ViewChild('scrollTable', { static: false }) scroll: ElementRef;
    listData$: Observable<DataListModel<DocumentTypeListModel>>;
    isLoading: boolean = false;
    searchQuery: QueryListModel = new QueryListModel({});
    private _formPopup: MatDialogRef<DocumentTypeFormComponent>;
    ...
    create(): void {
        const inputData: InputFormData<DocumentTypeModel> = new InputFormData<DocumentTypeModel>();
        inputData.data = new DocumentTypeModel({});
        inputData.action = 'add';
        this.openForm(inputData);
    }
    edit(rowData: DocumentTypeListModel): void {
        const inputData: InputFormData<DocumentTypeModel> = new InputFormData<DocumentTypeModel>();
        this._service.getData(rowData.id).pipe(takeUntil(this._unsubscribeAll)).subscribe((response: DocumentTypeModel) => {
            inputData.data = response;
            inputData.action = 'edit';
            this.openForm(inputData);
        });
    }
    deleteData(rowData: DocumentTypeListModel): void {
        Swal.fire({
            title: '',
    ...
    openForm(input: InputFormData<DocumentTypeModel>): void {
        this._formPopup = this._matDialog.open(DocumentTypeFormComponent, {
