...
import { DocumentTypeFormModel, DocumentTypeModel } from 'app/models/document-type.model';
...
export class DocumentTypeFormComponent implements OnInit, OnDestroy {
    ...
    constructor(
        private _matDialogRef: MatDialogRef<DocumentTypeFormComponent>,
        @Inject(MAT_DIALOG_DATA) private input: InputFormData<DocumentTypeModel>,
    ...
    saveData(): void {
        if (this.dataForm.valid) {
            const postData = new DocumentTypeFormModel(this.dataForm.getRawValue());
            Swal.fire({
                title: '',
