...
@Component({
    selector: 'agenda-objective-form',
    templateUrl: './form.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations,
})
export class AgendaObjectiveFormComponent implements OnInit, OnDestroy {
    @ViewChild('myForm') myForm: NgForm;
    action: string;
    dialogTitle: string;
    data: AgendaObjectiveModel;
    dataForm: UntypedFormGroup;
    kw: string;
    private _unsubscribeAll: Subject<any> = new Subject();
    constructor(
        private _matDialogRef: MatDialogRef<AgendaObjectiveFormComponent>,
        @Inject(MAT_DIALOG_DATA) private input: InputFormData<AgendaObjectiveModel>,
    ...
    saveData(): void {
        if (this.dataForm.valid) {
            const postData = new AgendaObjectiveModel(this.dataForm.getRawValue());
            Swal.fire({
                title: '',
