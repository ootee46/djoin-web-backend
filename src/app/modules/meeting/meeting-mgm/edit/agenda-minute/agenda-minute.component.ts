...
import { AgendaMinuteService } from './agenda-minute.service';
import { AgendaMinuteConfirmFormComponent } from './form/form.component';

@Component({
    selector: 'agenda-minute',
    templateUrl: './agenda-minute.component.html'
})
export class AgendaMinuteComponent implements OnInit, OnDestroy {
    ...
    requestId: number;
    private _formPopup: MatDialogRef<AgendaMinuteConfirmFormComponent>;
    private meetingId: number;
    private _unsubscribeAll: Subject<any> = new Subject<any>();
    private _query = new QueryListModel({});
    constructor(
        private _service: AgendaMinuteService,
    ...
    ngOnInit(): void {
        ...
        // });
        if(this._route.snapshot.paramMap.get('requestId')){
            try{
                this.requestId = parseInt(this._route.snapshot.paramMap.get('requestId'),10);
                if(this.requestId){
                    setTimeout(() => {
                        this.openDetail(this.requestId);
                    }, 100);
                }
            }catch{}
        }
    ...
    openConfirmForm(): void {
        ...
        input.data = obj;
        this._formPopup = this._matDialog.open(AgendaMinuteConfirmFormComponent, {
