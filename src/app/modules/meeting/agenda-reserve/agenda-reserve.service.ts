...
export class AgendaReserveService {
    private _listData: BehaviorSubject<DataListModel<AgendaRequestModel> | null> = new BehaviorSubject(null);
    ...
    private _relation: BehaviorSubject<MeetingRelationModel[] | null> = new BehaviorSubject(null);
    constructor(private _httpClient: HttpClient, private _globalService: GlobalService) {
    }
    get listData$(): Observable<DataListModel<AgendaRequestModel>> {
    ...
    get relation$(): Observable<MeetingRelationModel[]> {
        return this._relation.asObservable();
    }

    getDatas(query: QueryListModel): Observable<DataListModel<AgendaRequestModel>> {
        return this._httpClient.post<DataListModel<AgendaRequestModel>>(ENDPOINT.agendaRequest.search, query).pipe(
            tap((response: any) => {
                this._listData.next(new DataListModel<AgendaRequestModel>(response, AgendaRequestModel));
            })
        );
    }
    ...
    docSubmit(postData: AgendaRequestUpdateFormModel): Observable<any> {
        return this._httpClient.post<any>(`${ENDPOINT.agendaRequest.docSubmit}`, postData);
    }
    ...
    requestEdit(postData: ApproveFormModel): Observable<any> {
        return this._httpClient.post<any>(`${ENDPOINT.agendaRequest.requestEdit}`, postData);
    }
    requestCancel(postData: ApproveFormModel): Observable<any> {
        return this._httpClient.post<any>(`${ENDPOINT.agendaRequest.requestCancel}`, postData);
    }
    ...
    export(id: number): Observable<BookingExportModel> {
        return this._httpClient.post<BookingExportModel>(`${ENDPOINT.agendaRequest.export}/${id}`, null).pipe(
            switchMap((response: any) => {
                if (!response) {
                    return of(null);
                }
                else {
                    return of(new BookingExportModel(response));
                }
            })
        );
    ...
    getRelation(id: string): Observable<MeetingRelationModel[]> {
        return this._httpClient.post<MeetingRelationModel[]>(`${ENDPOINT.agendaRequest.getMeetingRelation}/${id}`, null).pipe(
            tap((response: any) => {
                this._relation.next(response.map((c: MeetingRelationModel) => new MeetingRelationModel(c)));
            })
        );
    }
