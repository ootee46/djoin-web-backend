...
export class EmployeeSearchService {
    private _listData: BehaviorSubject<DataListModel<LdapDataModel> | null> = new BehaviorSubject(null);
    private _ldapOption: BehaviorSubject<LdapOptionModel[] | null> = new BehaviorSubject(null);
    constructor(private _httpClient: HttpClient) {
    }
    get listData$(): Observable<DataListModel<LdapDataModel>> {
    ...
    getDatas(query: LdapSearchInputModel): Observable<DataListModel<LdapDataModel>> {
        return this._httpClient.post<DataListModel<LdapDataModel>>(ENDPOINT.service.ldapSearch, query).pipe(
            tap((response: any) => {
                this._listData.next(new DataListModel<LdapDataModel>(response, LdapDataModel));
            })
        );
    }
    ...
    getOptions(): Observable<LdapOptionModel[]> {
        return this._httpClient.post<LdapOptionModel[]>(ENDPOINT.common.ldapOption, null).pipe(
            ...
            tap((response: any) => {
                this._listData.next(response.map(c => new LdapOptionModel(c)));
            })
        );
    }
